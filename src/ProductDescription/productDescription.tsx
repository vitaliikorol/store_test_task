  ...
  return (
    <div className="ProductDescription PD">
      <aside className="PD__picture">
        <img className="PD__picture_img" src={item.product_images[1]} alt={item.article_no}/>
      </aside>
      <main className="PD__main">
        <button className="PD__main_button-back PD__btn">
          <img className="PD__btn_img" src="https://vitaliikorol.github.io/store_test_task/images/icons/up-arrow.png" alt="&larr;"/>
          НАЗАД
        </button>
        ...
        <div className="PD__main_buttons">
          <div className="PD__main_buttons_list-wrapper PDL">
            ...
            <button className="PDL__buttons" onClick={defineSliderVisible}>
              РАЗМЕР
              <img className="PDL__buttons_img"
                   src="https://vitaliikorol.github.io/store_test_task/images/icons/up-arrow.png" alt="&larr;"/>
            </button>
            <div className="PDL__buttons_list-wrapper">
              <ul className="PDL__buttons_list"
                  style={{top: sliderVisible ? '0' : `-${item.product_images.length * 30}px`}}>
                {item.sizes.map(el => (
                  <li onClick={() => defineSize(el)} key={el} className={cn(size === el ? 'PDL__buttons_item PDL__buttons_item-active' : 'PDL__buttons_item')}>{el}</li>
                ))}
              </ul>
            </div>

          </div>
          <button className="PDL__buttons PDL__buttons_cart" type="button">В КОРЗИНУ</button>
        </div>
        <div className="PD__description">
          <div className="PD__description_text">
            <p className="PD__description_text-main">{item.description}</p>
